...
function complexConvert(data) {
  ...
  const jsonData = JSON.stringify(myObj, (key, value) => {
...
function manipulateUrl(url) {
  ...
  let obj = {};
  obj.href = `${newUrl.href}`;
  obj.protocol = `${newUrl.protocol}`;
  obj.host = `${newUrl.host}`;
  obj.pathname = `${newUrl.pathname}`;
  obj.search = `${newUrl.search}`;
  obj.params = `${newUrl.searchParams}`;
...
function searchParamsURL(url) {
  ...
  const URLSearchParams = url1.searchParams.entries();
...
function checkURLParameters(url, params) {
  ...
  myObj.param1 = newUrl.searchParams.has("param1");
  myObj.param2 = newUrl.searchParams.has("param2");
  myObj.param3 = newUrl.searchParams.has("param3");
  myObj.param4 = newUrl.searchParams.has("param4");
  return myObj;
...
function getUrlData(url) {
  const newUrl = new URL(url);
  let obj = {};
  obj.origin = `${newUrl.origin}`;
  obj.hostname = `${newUrl.hostname}`;
  obj.port = `${newUrl.port}`;
  obj.username = `${newUrl.username}`;
  obj.password = `${newUrl.password}`;
...
function sortUrlParams(url) {
  ...
  let list = Array.from(newUrl.searchParams);
  list.sort();
  newUrl.searchParams.delete("param1");
  newUrl.searchParams.delete("param2");
...
function getURLValues(url) {
  ...
  let arrKeys = URLSearchParams.keys();
  let arrValue = [];
  for (const key of arrKeys) {
    arrValue.push(URLSearchParams.getAll(key));
  }
